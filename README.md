...
### Lens.org での特許データ作成例(無料)

[Lens.org](https://www.lens.org/lens/search/patent/structured) は無料登録で
**一度に50,000件までCSVエクスポート可能**(未登録は1,000件)。Query Text Editor に
以下を貼り付ける。

バランス版(推奨・タイトルまたは要約に主要キーワード):
...
高精度版(上記にロボット系CPC分類でANDを掛けノイズを除去):

```
(title:("humanoid robot" OR "humanoid robots" OR "physical AI" OR "physical artificial intelligence" OR "embodied AI" OR "embodied artificial intelligence" OR "embodied intelligence" OR "robot foundation model" OR "bipedal robot" OR "legged robot") OR abstract:("humanoid robot" OR "humanoid robots" OR "physical AI" OR "physical artificial intelligence" OR "embodied AI" OR "embodied artificial intelligence" OR "embodied intelligence" OR "robot foundation model" OR "bipedal robot" OR "legged robot")) AND class_cpc.symbol:(B25J* OR B62D57*)
```

- B25J = マニピュレータ・ロボット全般 / B62D57 = 歩行式走行車(脚式ロボット)
- 日本特許も英訳タイトル・要約で検索されるため英語キーワードで日米欧をカバーできる
- エクスポート前の推奨設定: 左パネルで Document Type = Patent Application + Granted Patent
  に絞り、「Group by Simple Family」でファミリー単位に重複排除
- Export → CSV を選択。出力の Display Key / Title / Application Date / Applicants 列を
  本ダッシュボードが自動判定して取り込む
